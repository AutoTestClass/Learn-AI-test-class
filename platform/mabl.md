...
* `Edit test`: 编辑测试用例，在录制操作的过程中，会有重复，或操作错误的动作，这些动作也会被录制下来。需要进行编辑和修改。
...
首先，我自己本地启动了一个Web系统，这样方便我自己修改页面。
...
点击侧边栏 `Tests` , 按钮 `+New browser test` 新建一条用例，填写用例基本信息。

![](../image/mabal_create_new_case.png)

然后，点击`Create test`按钮开始录制。
...
* 非颠覆式的创新：虽然 mabl 和 testim 有AI加持，但是：录制脚本、测试计划、测试结果 这些自动化测试流程并没有改变，元素定位只是自动化测试的一个点，我们还是要设计和组织用例，执行以及查看结果。对于自动化开发效率，以及维护成本并​没有质的提升。

* 价格：我在他们的官网上没有看到结果，需要人工咨询。参考国内的apifix的价格也不是个人能负担得起。有现成的具备自动化技术的测试工程师，当然是用开源的了。
